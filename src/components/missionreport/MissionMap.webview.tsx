...
const MissionMapBase: React.FC<Props> = ({
  ...
  // Throttle: rover position injections are capped at 5Hz (200ms) to avoid
  // overloading the React Native ↔ WebView bridge at 20Hz telemetry rates.
  const lastRoverInjectTs = useRef<number>(0);
  const ROVER_INJECT_THROTTLE_MS = 200;
...
<body>
  ...
  // Throttled to max 5Hz (200ms) to prevent overloading the WebView bridge.
  ...
  useEffect(() => {
    ...
    // ── 5Hz throttle ─────────────────────────────────────────────────────────
    const now = Date.now();
    if (now - lastRoverInjectTs.current < ROVER_INJECT_THROTTLE_MS) return;
    lastRoverInjectTs.current = now;
    // ─────────────────────────────────────────────────────────────────────────
...
export const MissionMapWebView = React.memo(MissionMapBase);
