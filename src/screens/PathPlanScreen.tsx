...
import {
  detectCorners,
  generateCornerExtensionWaypoints,
...
import {
  ...
  MapVisualization,
} from "../components/pathplan/MapVisualizationControls";
...
const PreviewRow = memo(({ item }: { item: PathPlanWaypoint }) => (
  ...
  >
    <Text style={{ flex: 0.4, color: colors.text, fontSize: 11 }}>
      {item.id}
    </Text>
    <Text
      style={{
      ...
      <View style={styles.bottomTableHeaderWrapper}>
        ...
        >
          ...
          <View style={styles.bottomHeaderRight}>
            ...
            {onClose && (
              <TouchableOpacity
                style={styles.bottomTableCloseBtn}
                onPress={onClose}
                activeOpacity={0.7}
              >
                <MaterialCommunityIcons
...
export default function PathPlanScreen({
  ...
  const pathPlanToastTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  useEffect(() => {
      ...
      (nextAppState: string) => {
        if (nextAppState === "active") {
          ...
          if (exportInProgressRef.current) {
            console.log(
              "[PathPlanScreen] App returned to foreground - resetting export flag",
            );
    ...
    return () => {
      ...
      console.log(
        "[PathPlanScreen] Component unmounting - all async operations cancelled",
      );
    };
  }, []);
  ...
  const [editingWaypoint, setEditingWaypoint] =
    useState<PathPlanWaypoint | null>(null);
  const [showEditDialog, setShowEditDialog] = useState(false);
  const [isBottomTableExpanded, setIsBottomTableExpanded] = useState(false);
  const [mapZoomTrigger, setMapZoomTrigger] = useState<{
    type: "in" | "out";
    timestamp: number;
  } | null>(null);
  ...
  const setIsVisMenuOpen = (v: boolean) =>
    setActivePanel(v ? "settings" : null);
  const setIsWidgetMenuOpen = (v: boolean) =>
    setActivePanel(v ? "widget" : null);
  ...
  const [localIsDrawingToolsVisible, setLocalIsDrawingToolsVisible] =
    useState(true);
  const isDrawingToolsVisible =
    propIsDrawingToolsVisible !== undefined
      ? propIsDrawingToolsVisible
      : localIsDrawingToolsVisible;
  const setIsDrawingToolsVisible =
    propSetIsDrawingToolsVisible || setLocalIsDrawingToolsVisible;

  const [localIsMissionOpsVisible, setLocalIsMissionOpsVisible] =
    useState(true);
  const isMissionOpsVisible =
    propIsMissionOpsVisible !== undefined
      ? propIsMissionOpsVisible
      : localIsMissionOpsVisible;
  const setIsMissionOpsVisible =
    propSetIsMissionOpsVisible || setLocalIsMissionOpsVisible;

  const [localIsStatisticsVisible, setLocalIsStatisticsVisible] =
    useState(true);
  const isStatisticsVisible =
    propIsStatisticsVisible !== undefined
      ? propIsStatisticsVisible
      : localIsStatisticsVisible;
  const setIsStatisticsVisible =
    propSetIsStatisticsVisible || setLocalIsStatisticsVisible;

  const [localIsBottomTableVisible, setLocalIsBottomTableVisible] =
    useState(true);
  const isBottomTableVisible =
    propIsBottomTableVisible !== undefined
      ? propIsBottomTableVisible
      : localIsBottomTableVisible;
  const setIsBottomTableVisible =
    propSetIsBottomTableVisible || setLocalIsBottomTableVisible;

  const [isRobotPositionVisible, setIsRobotPositionVisible] = useState(true);

  // GPS Failsafe state
  const [showFailsafeModeSelector, setShowFailsafeModeSelector] =
    useState(false);
  ...
  const [failsafeEvent, setFailsafeEvent] = useState<{
    wpDistCm: number;
    thresholdCm: number;
  } | null>(null);
  ...
  const [pathAssignmentMode, setPathAssignmentMode] = useState<
    "auto" | "manual"
  >("auto");
  ...
  const [pendingDXFImport, setPendingDXFImport] =
    useState<PreparedDXFImport | null>(null);
  const [showPrecisePathDialog, setShowPrecisePathDialog] = useState(false);
  const [precisePathPreview, setPrecisePathPreview] = useState<
    PathPlanWaypoint[] | null
  >(null);
  ...
  const handlePrecisePathApply = React.useCallback(
    (optimized: PathPlanWaypoint[]) => {
  ...
  const [gpsInputA, setGpsInputA] = useState<{ lat: string; lon: string }>({
    lat: "",
    lon: "",
  });
  const [gpsInputB, setGpsInputB] = useState<{ lat: string; lon: string }>({
    lat: "",
    lon: "",
  });
  const [showCornerExtensionDialog, setShowCornerExtensionDialog] =
    useState(false);
  ...
  const [measurePoints, setMeasurePoints] = useState<
    { lat: number; lon: number; seq: number; waypointId?: number }[]
  >([]);
  ...
  const handleMeasureWaypointSelect = React.useCallback(
    (id: number) => {
      ...
      setMeasurePoints((prev) => {
        ...
        const newPts = [
          ...prev,
          { lat: wp.lat, lon: wp.lon, seq: prev.length + 1, waypointId: id },
        ];
        if (newPts.length === 2) {
      ...
      try {
        ...
        if (savedUIState) {
          if (savedUIState.selectedWaypoint !== undefined) {
            ...
            console.log(
              "[PathPlanScreen] 📂 Restored selected waypoint:",
              savedUIState.selectedWaypoint,
            );
          }
          if (savedUIState.isDrawingToolsCollapsed !== undefined) {
            setIsDrawingToolsCollapsed(savedUIState.isDrawingToolsCollapsed);
            console.log(
              "[PathPlanScreen] 📂 Restored drawing tools collapsed:",
              savedUIState.isDrawingToolsCollapsed,
            );
          }
        ...
        if (savedMapVisualization) {
          ...
          console.log(
            "[PathPlanScreen] 📂 Restored map visualization settings",
          );
        }
      } catch (error) {
        console.error(
          "[PathPlanScreen] Failed to load persisted state:",
          error,
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const handleServoSuppressed = (event: any) => {
      console.log(
        "[PathPlanScreen] 🚫 Servo suppressed event received:",
      ...
      if (gpsFailsafeMode === "strict") {
        ...
        console.log(
          "[PathPlanScreen] ℹ️ RELAX MODE: Showing failsafe notification",
        );
        console.log(
          "[PathPlanScreen] ℹ️ Mission continues with spray suppressed",
        );
  ...
  const debouncedAutoSave = useCallback(() => {
    ...
    if (homePosition) {
      autoSaveTimersRef.current.homePosition = setTimeout(() => {
        PersistentStorage.saveHomePosition(homePosition).catch((error) => {
          console.error(
            "[PathPlanScreen] Failed to persist home position:",
            error,
          );
        });
      }, 500);
    ...
    autoSaveTimersRef.current.drawingMode = setTimeout(() => {
      PersistentStorage.saveDrawingMode(isDrawingMode).catch((error) => {
        console.error(
          "[PathPlanScreen] Failed to persist drawing mode:",
          error,
        );
      });
    }, 300);
    ...
    if (activeDrawingTool) {
      autoSaveTimersRef.current.activeTool = setTimeout(() => {
        PersistentStorage.saveActiveTool(activeDrawingTool).catch((error) => {
          console.error(
            "[PathPlanScreen] Failed to persist active tool:",
            error,
          );
        });
      }, 300);
    ...
    autoSaveTimersRef.current.mapVisualization = setTimeout(() => {
      PersistentStorage.saveMapVisualization(mapVisualization).catch(
        (error) => {
          console.error(
            "[PathPlanScreen] Failed to persist map visualization settings:",
            error,
          );
        },
      );
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      PersistentStorage.savePathPlanUIState({
        ...
        console.error(
          "[PathPlanScreen] Failed to persist selected waypoint:",
          error,
        );
      });
    }, 500);
  ...
  const handleMapPress = (coord: { latitude: number; longitude: number }) => {
    ...
    if (activeDrawingTool === "measure") {
      setMeasurePoints((prev) => {
        ...
        const newPoints = [
          ...prev,
          { lat: coord.latitude, lon: coord.longitude, seq: prev.length + 1 },
        ];
  ...
  const handleWaypointClick = (id: number) => {
    ...
    if (manualPathConnections.includes(id)) {
      showPathPlanToast(
        "info",
        "Already Connected",
        `Marking point #${id} is already in your path.`,
      );
      return;
    }
  ...
  const openManualConnection = useCallback(() => {
    if (waypoints.length < 2) {
      showPathPlanToast(
        "error",
        "Manual Connect",
        "Add at least 2 marking points before connecting a path.",
      );
      return;
    }
  ...
  const handleManualConnectionDelete = useCallback(
    (deletedWaypointIds: number[]) => {
      if (deletedWaypointIds.length === 0) return;
      const deleted = new Set(deletedWaypointIds);
      const remaining = waypoints
        .filter((wp) => !deleted.has(wp.id))
        .map((wp, index) => ({
          ...wp,
          id: index + 1,
          pile: wp.pile || String(index + 1),
  ...
  const handleToggleMark = React.useCallback(
    (id: number, newMarkValue: boolean) => {
  ...
  const handleAddWaypoints = (
    ...
    const newWaypoints = coords.map((coord, index) => {
      ...
      const lastWp =
        index === 0 && waypoints.length > 0
          ? waypoints[waypoints.length - 1]
          : null;
      const prevWp =
        index > 0
          ? {
              lat: coords[index - 1].latitude,
              lon: coords[index - 1].longitude,
            }
          : lastWp;
    ...
    showPathPlanToast(
      "success",
      "Marking Points Added",
      `${newWaypoints.length} marking points added to mission`,
    );
  };
  ...
  const handleTextAnnotation = (
    ...
    if (textCoords.length === 0) {
      showPathPlanToast(
        "error",
        "No Marking Points",
        "Unable to generate marking points for the given text.",
      );
      return;
    }
    ...
    let lastValidWp: { lat: number; lon: number } | null =
    ...
    showPathPlanToast(
      "success",
      "Text Path Created",
      `${newWaypoints.length} marking points generated for "${text}"`,
    );
  };
  ...
  const handleDrawingComplete = (
    ...
    if (DEBUG_LOG)
      console.log(
        "[PathPlan] Drawing completed with",
        coords.length,
        "coordinates",
      );
    ...
    if (newWaypoints.length > 0) {
      if (DEBUG_LOG)
        console.log(
          "[PathPlan] Adding",
          newWaypoints.length,
          "waypoints from drawing",
        );
      recordAndApply([...waypoints, ...newWaypoints]);
      showPathPlanToast(
        "success",
        "Drawing Complete",
        `${newWaypoints.length} marking points created from your drawing`,
      );
    } else {
      showPathPlanToast(
        "error",
        "No Marking Points",
        "Drawing did not generate any marking points. Try drawing a longer path.",
      );
    }
  ...
  const reverseWaypointOrder = useCallback(
    (inputWaypoints: PathPlanWaypoint[]): PathPlanWaypoint[] => {
  ...
  const handleReverseUploadPreviewWaypoints = useCallback(() => {
    setUploadPreviewWaypoints((prev) =>
      prev ? reverseWaypointOrder(prev) : prev,
    );
  ...
  const [cornerExtensionOptions, setCornerExtensionOptions] =
    useState<CornerExtensionOptions>(DEFAULT_EXTENSION_OPTIONS);

  const cornerDetectionResult = useMemo(() => {
    if (waypoints.length < 3) return { count: 0, shortWarnings: 0 };
    const corners = detectCorners(
      waypoints,
      cornerExtensionOptions.turnAngleThreshold,
      cornerExtensionOptions.extensionDistance,
    );
    return {
  ...
  const handleApplyCornerExtension = useCallback(
    (options: CornerExtensionOptions) => {
      if (waypoints.length < 3) {
        showPathPlanToast(
          "error",
          "Not Enough Waypoints",
          "At least 3 waypoints are required for corner extension.",
        );
        return;
      }
      ...
      if (extended.length === waypoints.length) {
        showPathPlanToast(
          "info",
          "No Corners Detected",
          "No corners above the threshold were found. No extension points added.",
        );
        return;
      }
  ...
  const validateFileExtension = (filename: string): boolean => {
    ...
    if (DEBUG_LOG)
      console.log("[PathPlan] validateFileExtension:", { filename, ext });
  ...
  const validateWaypoint = (wp: any, index: number): boolean => {
    ...
    if (lat < -90 || lat > 90) {
      throw new Error(
        `Invalid latitude ${lat} at waypoint ${index + 1}. Must be between -90 and 90.`,
      );
    }

    if (lon < -180 || lon > 180) {
      throw new Error(
        `Invalid longitude ${lon} at waypoint ${index + 1}. Must be between -180 and 180.`,
      );
    }
  ...
  const parseQGCWaypoints = (content: string): PathPlanWaypoint[] => {
    ...
    if (lines.length === 0 || !lines[0].startsWith("QGC WPL")) {
      throw new Error(
        'Invalid QGC waypoint format. File must start with "QGC WPL 110".',
      );
    }
    ...
    dataLines.forEach((line, idx) => {
      ...
      if (parts.length < 11) {
        throw new Error(
          `Invalid QGC waypoint format at line ${idx + 2}. Expected at least 11 tab-separated fields.`,
        );
      }
    ...
    if (DEBUG_LOG)
      console.log(
        "[PathPlan] parseQGCWaypoints -> parsed",
        waypoints.length,
        waypoints.slice(0, 6),
      );
  ...
  const handleDXFUpload = (content: string, fileName: string) => {
    try {
      if (DEBUG_LOG)
        console.log(
          "[PathPlan] DXF file selected:",
          fileName,
          "— preparing import choices",
        );
  ...
  const applyPreparedDXFAsWaypoints = (prepared: PreparedDXFImport) => {
    ...
    if (imported.length === 0) {
      showPathPlanToast(
        "error",
        "No Waypoints",
        "No waypoint-capable geometry was found in this DXF.",
      );
      return;
    }
    ...
    if (pathAssignmentMode === "manual") {
      ...
      showPathPlanToast(
        "info",
        "DXF Waypoints Imported",
        `${withDistances.length} marking points imported. Connect them on the canvas.`,
        5000,
      );
    } else {
      recordAndApply(withDistances);
      showPathPlanToast(
        "success",
        "DXF Waypoints Imported",
        `${withDistances.length} marking points placed near the rover.`,
      );
    }
  };
  ...
  const applyPreparedDXFAsEntities = (prepared: PreparedDXFImport) => {
    const importedEntities = importDXFAsEntities(
      prepared,
      getDXFPlacementOrigin(),
    );
    if (importedEntities.length === 0) {
      showPathPlanToast(
        "error",
        "No Entities",
        "No renderable DXF entities were found.",
      );
      return;
    }
    ...
    showPathPlanToast(
      "success",
      "DXF Entities Imported",
      `${importedEntities.length} shapes placed near the rover.`,
    );
  };
  ...
  const handleExportMission = async (
    ...
    try {
      ...
      if (DEBUG_LOG)
        console.log("[PathPlan] Exporting mission as", format, "to", fileUri);
      ...
      if (DEBUG_LOG)
        console.log("[PathPlan] File written successfully:", fileUri);
      ...
      const downloadPromise = downloadFileToDevice(
        fileUri,
        filename,
        getMimeType(format),
      );
      const downloadTimeout = new Promise<boolean>((_, reject) => {
        const timer = setTimeout(
          () =>
            reject(new Error("Permission/download timeout after 30 seconds")),
          30000,
        );
        addTimer(timer);
      });
        ...
        if (
          ...
          Alert.alert(
            ...
            [
              ...
              {
                text: "Retry",
                onPress: () => handleExportMission(format, content, filename),
              },
            ],
          );
        }
  ...
  const handleLoadFromController = async () => {
    ...
    if (!mountedRef.current) {
      console.warn(
        "[PathPlan] Component unmounted, aborting load from controller",
      );
      return;
    }
    ...
    try {
      ...
      if (!Array.isArray(wps)) {
        throw new Error(
          "Invalid response format - waypoints should be an array",
        );
      }
      ...
      wps.forEach((wp: any, idx: number) => {
        try {
          ...
          errors.push(
            `Waypoint ${idx + 1}: ${wpError instanceof Error ? wpError.message : "Parse error"}`,
          );
        }
      });
      ...
      if (!mountedRef.current) {
        console.warn(
          "[PathPlan] Component unmounted during waypoint processing",
        );
        return;
      }
      ...
      console.log(
        `[PathPlan] Loaded ${withDistances.length} waypoints from controller`,
      );
    } catch (err) {
  ...
  const handleRequestUpload = async () => {
    if (isUploadingRef.current) {
      ...
      if (DEBUG_LOG)
        console.log(
          "[PathPlan] Upload already in progress, ignoring re-entrant call",
        );
      return;
    }
    ...
    try {
      ...
      if (DEBUG_LOG)
        console.log("[PathPlan] DocumentPicker response full:", res);
      if (DEBUG_LOG)
        console.log(
      ...
      if (DEBUG_LOG)
        console.log(
          "[PathPlan] DocumentPicker response.type:",
          (res as any).type,
        );
      ...
      if ((res as any).type === "cancel" || (res as any).cancelled === true) {
        if (DEBUG_LOG)
          console.log("[PathPlan] DocumentPicker was cancelled by user");
        return;
      }
      ...
      if ((res as any).type !== "success" && !(res as any).assets) {
        if (DEBUG_LOG)
          console.log(
            "[PathPlan] DocumentPicker failed or cancelled - type:",
            (res as any).type,
          );
        return;
      }
      ...
      if (
        ...
        if (DEBUG_LOG)
          console.log("[PathPlan] Using new DocumentPicker format:", {
            uri,
            name,
          });
        ...
        if (DEBUG_LOG)
          console.log("[PathPlan] Using old DocumentPicker format:", {
            uri,
            name,
          });
      } else {
        if (DEBUG_LOG)
          console.log("[PathPlan] Unexpected DocumentPicker response format");
        showPathPlanToast(
          "error",
          "Upload Error",
          "Unable to read file selection. Please try again.",
        );
        return;
      }

      if (DEBUG_LOG)
        console.log("[PathPlan] Final extracted - uri:", uri, "name:", name);
      ...
      if (DEBUG_LOG)
        console.log("[PathPlan] Selected file metadata:", { name, ext });
      ...
      try {
        ...
        if (DEBUG_LOG)
          console.log("[PathPlan] File content length:", content?.length ?? 0);
      } catch (readError) {
      ...
      switch (ext) {
        ...
        case "csv": {
          ...
          if (csvRows > 1000) {
            if (DEBUG_LOG)
              console.log(
                "[PathPlan] Using chunked parser for",
                csvRows,
                "rows",
              );
      ...
      if (!parsed || parsed.length === 0) {
        showPathPlanToast(
          "error",
          "Import Failed",
          "No valid marking points were found in the file.",
        );
        return;
      }
      ...
      if (DEBUG_LOG)
        console.log(
          "[PathPlan] Waypoints with distances:",
          waypointsWithMark.length,
        );
      ...
      if (DEBUG_LOG)
        console.log("[PathPlan] Validation result:", {
      ...
      Alert.alert("Mission Import Failed", userMessage, [
        { text: "OK", style: "default" },
        {
          text: "Try Again",
          onPress: () => {
  ...
  const handleGPSSubmit = () => {
    ...
    if (isNaN(latA) || isNaN(lonA) || isNaN(latB) || isNaN(lonB)) {
      showPathPlanToast(
        "error",
        "Invalid GPS",
        "Enter valid latitude and longitude values for both points.",
      );
      return;
    }
  ...
  const handleApplyCADAlignment = () => {
    ...
    if (waypoints.length === 0) {
      showPathPlanToast(
        "error",
        "No Waypoints",
        "No convertible entities found. The DXF may contain only unsupported entity types.",
      );
      return;
    }
    ...
    if (pathAssignmentMode === "manual") {
      ...
      showPathPlanToast(
        "success",
        "CAD Import Complete",
        `Successfully georeferenced ${waypoints.length} marking points.`,
      );
    }
  ...
  const renderCADModeUI = () => {
    ...
    return (
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={handleCancelCADMode}
      >
        <View style={cadStyles.modalOverlay}>
          <View style={cadStyles.modalContent}>
            ...
            <View style={cadStyles.header}>
              ...
              <TouchableOpacity
                onPress={handleCancelCADMode}
                style={cadStyles.closeButton}
              >
                <MaterialCommunityIcons
                  name="close"
                  size={20}
            ...
            {state === "error" && errorMessage && (
              <View style={cadStyles.errorBox}>
                ...
                <TouchableOpacity
                  style={cadStyles.errorRetryButton}
                  onPress={handleCancelCADMode}
                >
              ...
              >
                ...
                >
                  <MaterialCommunityIcons
                    ...
                    color={colors.text}
                  />
                  <Text style={cadStyles.gpsComputeText}>
                    Quick Align (Rover Position)
                  </Text>
                </TouchableOpacity>
              </View>
            )}
            ...
            {showGPSInput && (
              <View style={cadStyles.gpsInputContainer}>
                <Text style={cadStyles.sectionTitle}>
                  Enter GPS Reference Points
                </Text>
                ...
                <View style={cadStyles.gpsRow}>
                  ...
                  >
                    <Text style={cadStyles.gpsInputText}>
                      {gpsInputA.lat || "tap to use current"}
                    </Text>
                  </TouchableOpacity>
                  ...
                  >
                    <Text style={cadStyles.gpsInputText}>
                      {gpsInputB.lon || "tap to use current"}
                    </Text>
                  </TouchableOpacity>
                </View>
                ...
                <View style={cadStyles.gpsButtons}>
                  <TouchableOpacity
                    style={cadStyles.gpsComputeButton}
            ...
            {state === "computed" && computedWaypoints && (
              <View style={cadStyles.resultBox}>
                ...
                <Text style={cadStyles.resultCount}>
                  {computedWaypoints.length} waypoints generated
                </Text>

                <View style={cadStyles.resultActions}>
                  <TouchableOpacity
                    style={cadStyles.applyButton}
                    onPress={handleApplyCADAlignment}
                  >
                    <MaterialCommunityIcons
                      name="check"
                      size={18}
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      if (cadAlignment.cadModel) {
                        ...
                        cadAlignment.loadDXF(
                          JSON.stringify(cadAlignment.cadModel),
                        );
                      }
                    }}
                  >
    ...
    return (
      ...
      >
        <BottomTableHeader
          ...
          onToggleExpand={() =>
            setIsBottomTableExpanded(!isBottomTableExpanded)
          }
        ...
        {isBottomTableExpanded && (
          <View style={styles.bottomTableContent}>
            ...
            <View style={styles.bottomTableColHeaderRow}>
              ...
              <Text style={[styles.colHeaderCell, { flex: 1.5 }]}>
                LATITUDE
              </Text>
              <Text style={[styles.colHeaderCell, { flex: 1.5 }]}>
                LONGITUDE
              </Text>
              <Text style={[styles.colHeaderCell, { width: 120 }]}>
                ALTITUDE (m)
              </Text>
              <Text style={[styles.colHeaderCell, { width: 120 }]}>
                DISTANCE (m)
  ...
  return (
    ...
    >
        ...
        {isMapFullscreen ? (
          ...
          <View style={styles.fullscreenMap}>
            ...
            <PathPlanMap
              ...
              onWaypointDrag={
                showPrecisePathDialog ? undefined : handleWaypointDrag
              }
              onWaypointClick={
                showPrecisePathDialog ? undefined : handleWaypointClick
              }
              onAddWaypoints={
                showPrecisePathDialog ? undefined : handleAddWaypoints
              }
              roverPosition={
                roverPosition
                  ? {
              ...
              activeDrawingTool={
                showPrecisePathDialog ? null : activeDrawingTool
              }
              onDrawingComplete={
                showPrecisePathDialog ? undefined : handleDrawingComplete
              }
              isDrawingMode={false}
          ...
          <>
            ...
            {isDrawingToolsVisible && (
              <DraggableCard
                style={styles.floatingDrawingToolsWrapper}
                handleType="custom"
              >
                <DrawingToolsPanel
                  activeDrawingTool={activeDrawingTool}
                  ...
                  onShowCornerExtension={() =>
                    setShowCornerExtensionDialog(true)
                  }
                  ...
                  onToggleCollapse={() =>
                    setIsDrawingToolsCollapsed(!isDrawingToolsCollapsed)
                  }
            ...
            {isMissionOpsVisible && (
              <DraggableCard
                ...
                handleType="custom"
                onLayout={(e: any) =>
                  setOpsPanelHeight(e.nativeEvent.layout.height)
                }
              >
                <MissionOpsPanel
                  waypoints={waypoints}
            ...
            {isStatisticsVisible && (
              <DraggableCard
                ...
                handleType="custom"
                onLayout={(e: any) =>
                  setStatsPanelHeight(e.nativeEvent.layout.height)
                }
              >
                <MissionStatistics
                  waypoints={
                    isConnectingPath && manualPathConnections.length > 0
            ...
            {isRobotPositionVisible && (
              <DraggableCard
                style={[
                  ...
                  opsPanelHeight > 0 && {
                    top:
                      ...
                      opsPanelHeight +
                      20 +
                      (isStatisticsVisible && statsPanelHeight > 0
                        ? statsPanelHeight + 20
                        : 0),
                  },
                ]}
                handleType="custom"
                onLayout={(e: any) =>
                  setRobotPanelHeight(e.nativeEvent.layout.height)
                }
              >
                <RobotPositionPanel
                  roverPosition={
      ...
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => setShowUploadPreview(false)}
      >
        <View
          style={{
            flex: 1,
        ...
        >
          ...
          >
            ...
            <Text style={{ color: colors.textSecondary, marginBottom: 12 }}>
              {uploadPreviewName} —{" "}
              {uploadPreviewWaypoints ? uploadPreviewWaypoints.length : 0}{" "}
              marking points
            </Text>
            ...
            >
              ...
              <View style={{ flexDirection: "row", gap: 10 }}>
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    borderWidth: 2,
                    borderColor:
                      pathAssignmentMode === "auto" ? "#4ADE80" : "transparent",
                  }}
                >
                  <Text
                    style={{
              ...
              {pathAssignmentMode === "auto" && (
                <TouchableOpacity
                  ...
                  disabled={
                    !uploadPreviewWaypoints || uploadPreviewWaypoints.length < 2
                  }
                  style={{
                    marginTop: 10,
                    paddingVertical: 10,
                    paddingHorizontal: 14,
                    backgroundColor:
                      !uploadPreviewWaypoints ||
                      uploadPreviewWaypoints.length < 2
                        ? "#475569"
                        : colors.blueBtn,
                    borderRadius: 8,
                    alignItems: "center",
              ...
              {pathAssignmentMode === "manual" && (
                ...
                >
                  <Text style={{ color: colors.textSecondary, fontSize: 11 }}>
                    💡 Tip: In manual mode, waypoints will appear on the map
                    without connections. Click waypoints in order to connect
                    your custom path.
                  </Text>
                </View>
              )}
            ...
            {uploadPreviewValidationErrors.length > 0 && (
              ...
              >
                ...
                <ScrollView style={{ maxHeight: 120 }}>
                  {getCriticalErrors(uploadPreviewValidationErrors).length >
                    0 && (
                    <View style={{ marginBottom: 8 }}>
                      ...
                      {getCriticalErrors(uploadPreviewValidationErrors)
                        .slice(0, 3)
                        .map((err, idx) => (
                          <Text
                            key={`error-${err.message}-${idx}`}
                            style={{
                              color: "#ff6464",
                              fontSize: 11,
                              marginBottom: 2,
                      ...
                      {getCriticalErrors(uploadPreviewValidationErrors).length >
                        3 && (
                        <Text style={{ color: "#ff6464", fontSize: 11 }}>
                          ... and{" "}
                          {getCriticalErrors(uploadPreviewValidationErrors)
                            .length - 3}{" "}
                          more
                  ...
                  {getWarnings(uploadPreviewValidationErrors).length > 0 && (
                    <View>
                      ...
                      {getWarnings(uploadPreviewValidationErrors)
                        .slice(0, 3)
                        .map((warn, idx) => (
                          <Text
                            key={`warning-${warn.message}-${idx}`}
                            style={{
                              color: "#ffaa00",
                              fontSize: 11,
                              marginBottom: 2,
                      ...
                      {getWarnings(uploadPreviewValidationErrors).length >
                        3 && (
                  ...
                  if (uploadPreviewWaypoints) {
                    ...
                    if (warnings.length > 0) {
                      Alert.alert(
                        ...
                        [
                          { text: "Cancel" },
                          {
                            text: "Proceed Anyway",
                            onPress: () => {
                              const sanitized = sanitizeWaypointsForUpload(
                                uploadPreviewWaypoints,
                              );
                              ...
                              if (pathAssignmentMode === "manual") {
                                if (DEBUG_LOG)
                                  console.log(
                                    "[PathPlan] Importing waypoints in MANUAL mode (with warnings):",
                                    sanitized.length,
                                  );
                                ...
                                if (DEBUG_LOG)
                                  console.log(
                                    "[PathPlan] Applying imported waypoints (Proceed with warnings):",
                                    sanitized.length,
                                    sanitized.slice(0, 3),
                                  );
                                requestAnimationFrame(() => {
                      ...
                      if (pathAssignmentMode === "manual") {
                        ...
                        if (DEBUG_LOG)
                          console.log(
                            "[PathPlan] Importing waypoints in MANUAL mode:",
                            sanitized.length,
                          );
                        requestAnimationFrame(() => {
                          recordAndApply(sanitized);
                        ...
                        if (DEBUG_LOG)
                          console.log(
                            "[PathPlan] Applying imported waypoints (Proceed clean):",
                            sanitized.length,
                            sanitized.slice(0, 3),
                          );
                        requestAnimationFrame(() => {
        ...
        defaultCenter={
          roverPosition
            ? {
                ...
                alt: telemetry.global?.alt_rel ?? 30,
              }
            : undefined
        }
      />
      ...
      >
        ...
        >
          ...
          >
            <View style={styles.dxfChoiceHeader}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.dxfChoiceSubtitle} numberOfLines={1}>
                  {pendingDXFImport?.fileName ?? "drawing.dxf"} ·{" "}
                  {pendingDXFImport?.model.entities.length ?? 0} entities
                </Text>
              </View>
              <TouchableOpacity
                style={styles.dxfChoiceClose}
                onPress={() => setPendingDXFImport(null)}
                activeOpacity={0.7}
              >
                <MaterialCommunityIcons
            ...
            <TouchableOpacity
              style={styles.dxfChoiceOption}
              onPress={handleImportDXFAsWaypoints}
              activeOpacity={0.78}
            >
              <MaterialCommunityIcons
              ...
              <View style={styles.dxfChoiceOptionText}>
                <Text style={styles.dxfChoiceOptionTitle}>
                  Import Waypoints
                </Text>
                <Text style={styles.dxfChoiceOptionSub}>
                  Convert DXF vertices into mission points near rover.
                </Text>
              </View>
              <MaterialCommunityIcons
                name="chevron-right"
            ...
            <TouchableOpacity
              style={styles.dxfChoiceOption}
              onPress={handleImportDXFAsEntities}
              activeOpacity={0.78}
            >
              <MaterialCommunityIcons
                name="vector-polyline"
              ...
              <View style={styles.dxfChoiceOptionText}>
                ...
                <Text style={styles.dxfChoiceOptionSub}>
                  Render DXF lines, arcs, points, and labels as shapes.
                </Text>
              </View>
              <MaterialCommunityIcons
                name="chevron-right"
      ...
      <CADDrawingCanvasNew
        ...
        onSaveDXF={(dxfContent, ents, importMode) => {
          ...
          try {
            ...
            console.log(
              "[PathPlan] CAD drawing imported from DXF save",
              importMode ?? "pending",
              ents.length,
              "entities,",
              dxfContent.length,
            ...
            console.error(
              "[PathPlan] CAD drawing DXF preparation failed:",
              error,
            );
            showPathPlanToast(
              "error",
              ...
              error instanceof Error
                ? error.message
                : "Unable to prepare the drawing for import.",
              5000,
            );
          }
        }}
      ...
      >
        <View style={{ flex: 1, backgroundColor: colors.primary }}>
          ...
          >
            <TouchableOpacity
              onPress={handleCloseManualControl}
      ...
      <FailsafeModeSelector
        ...
        disabled={["running", "RUNNING", "active", "ACTIVE"].includes(
      ...
      <EditWaypointDialog
        ...
        onSave={(updatedWp) => {
          const updatedList = waypoints.map((wp) =>
            wp.id === updatedWp.id ? updatedWp : wp,
          );
